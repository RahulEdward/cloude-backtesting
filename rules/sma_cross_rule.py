...
Generates variants for each SMA period pair.
"""
...
class SMACrossRule(BaseRule):
    ...
    @property
    def name(self) -> str:
        return f"SMA Cross({self.fast_period}/{self.slow_period})"
    ...
    def generate_signals(self, df: pd.DataFrame) -> tuple[pd.Series, pd.Series]:
        ...
        logger.debug(f"Generating SMA cross signals ({self.fast_period}/{self.slow_period})")
        ...
        logger.debug(f"SMA cross signals: {entries.sum()} entries, {exits.sum()} exits")
